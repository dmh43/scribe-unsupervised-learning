...
\KwResult{$S_{greedy}$ }
 $S_0=\emptyset$\;
 \While{$|S_i| \leq k$}{
  $\displaystyle S_{i+1} = S_i \, \cup \, \argmax_{s \in \{S_g \setminus S_i\}} \bigg\{ \Delta(s\setminus S_i) \bigg\}$ \;
%  \eIf{condition}{
%   instructions1\;
%   instructions2\;
%   }{
...
\newpage
\begin{claim}
$\displaystyle f(S^* \cup S_i) = f(S_i) + \sum_{j=1}^{k} \Delta \bigg(v_j^* \setminus \big\{S_i \cup \{v_1^*,v_2^*,\ldots,v_{j-1}^*\}\big\}\bigg)$

\begin{subproof}[Subproof]
Expand:
%%%%
\begin{flalign*}
\displaystyle f(S^* \cup S_i) &= f(S_i) + \Delta \big(v_1^* \setminus S_i \big)
+\Delta \bigg(v_2^* \setminus \big\{S_i \cup v_1^*\big\}\bigg)
+\cdots 
+\Delta \bigg(v_k^* \setminus\big\{S_i \cup \{v_1^*,v_2^*,\ldots,v_{k-1}^*\}\big\}\bigg) &\\
%%%%
...
%%%%
\begin{flalign*}
f(S^*) \leq f(S^* \cup S_i) &=  f(S_i) + \sum_{j=1}^{k} \Delta \bigg(v_j^* \setminus \big\{S_i \cup \{v_1^*,v_2^*,\ldots,v_{j-1}^*\}\big\}\bigg) & &\\
&\leq f(S_i) + \sum_{j=1}^{k} \Delta (v_j^* \setminus S_i) &\text{(by submodularity)} &\\
